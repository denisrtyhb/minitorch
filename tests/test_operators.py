...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    sig_a = sigmoid(a)
    sig_minus_a = sigmoid(-a)
    assert 0.0 <= sig_a and sig_a <= 1.0
    assert_close(1 - sig_a, sig_minus_a)
    assert_close(sigmoid(0.0), 0.5)
    print(a, -a)
    if abs(a) < 1e-7:
        return
    elif a < -a:
        assert sig_a < sig_minus_a
    elif a > -a:
        assert sig_a > sig_minus_a
...
def test_other(a: float, b: float) -> None:
    ...
    """
    # TODO: Implement for Task 0.2.
    assert exp(a) > 0 and exp(b) > 0 
    if a > b:
        a, b = b, a
    assert abs(a - b) <= 1e-7 or exp(a) <= exp(b)

    if a > 0 and b > 0:
        assert abs(a - b) <= 1e-7 or log(a) <= log(b)
